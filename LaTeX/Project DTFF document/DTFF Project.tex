\documentclass[11pt,a4paper,openany,oneside]{article}%Memoir class for special chapters style (openany to have the chapters open in any page number (odd and not), onseside to get the pagenumber always on one side of the page)
\usepackage[utf8]{inputenc}%alphabetic type 
\usepackage[T1]{fontenc}%same
%pagenumber always right bottom{begin}
\usepackage{graphicx}
\usepackage{multirow}%have the possibility to use more rows in one column for just one cell instad of N
\usepackage{parskip}
\usepackage[margin=10pt, font=small, labelfont=bf, labelsep=endash]{caption}%To costumize caption captionposition not working
\usepackage{bm}%for thick font in math mode
\usepackage{floatrow} %to have the caption on top
\floatsetup[table]{capposition=top}%to have the caption on top
\floatsetup[figure]{capposition=top}%to have the caption on top
\usepackage{float}%for having the position of the tables fix with the command H instead of hbpt!
\usepackage{amsmath}
\usepackage{rotating}%For table rotation
%%%%%%% For thick lines in tables: to use: " for vertical thick lines and \thickhline for thick horizontal line
\usepackage{array}
\makeatletter
\newcommand{\thickhline}{%
...
\makeatother
%%%%%%%%%%%%%%% End of the thick line code

\usepackage[bottom=1.0in,top=1.0in,left=2.0cm,right=2.0cm]{geometry}
\usepackage[export]{adjustbox}
%pagenumber always right bottom{end}
\usepackage{enumitem}%letters enumaration list 
\usepackage{abstract}%for abstract section 
\usepackage{color}%For chapter style
% \usepackage[left=1in, right=1in]{geometry} %Helps to structure the paper layout.
\usepackage{caption}%For caption details for example 10pt distance
\usepackage{hyperref}%direct link for lab and ref 
...
\usepackage{pdflscape}%for landscape tabularx 
\usepackage{lscape}%for landscape tabularx
...
\usepackage{titlesec}%modify the spacing before and after the sections 
\titlespacing*{\section}
{0pt}{5.5ex plus 1ex minus .2ex}{5.3ex plus .2ex}
\titlespacing*{\subsection}
{0pt}{5.5ex plus 1ex minus .2ex}{4.3ex plus .2ex}
%CHAPTER SYLE code lines
%From Valuation course in case vanting to change




%END CHAPTER STYLE 
%ALternative to the classic titlepage USING THIS
\newcommand{\titlePI}{\begingroup %Create the command for including the title page in the document.
\noindent
\newgeometry{bottom=0.1cm,left=1.2cm,right=1.2cm}
\includegraphics[width=1.1\textwidth,set height=8cm,left=\textwidth]{Tech.png}\\[1\baselineskip]
\begin{center}
 \Huge{\textbf{Event Study: Abnormal Return Evaluation of Three Widely Publicized Mergers }}\\[\baselineskip] %Title.   
\end{center}
\vspace{0.9in}
\noindent
\scshape %Small caps.
\rule{\textwidth}{1.6pt}
\large \textbf{\Large Prof. Dr. Per Östberg}\\[\parskip]
Department of Banking \& Finance\\[2\baselineskip]
{\Large Takeovers, Restructuring, and Corporate Governance}\\[\parskip]
Event Study I | October 22, 2020\\[2\baselineskip]
\noindent\begin{tabularx}{\textwidth}{@{}lXs}
\Large Paride Iadonisi && 16-703-001\\
\Large Alexander Christian Keller && 16-704-603\\
\Large Lionel Meise && 15-703-903\\
\end{tabularx}
\rule{\textwidth}{1.6pt}
\restoregeometry
\endgroup}
